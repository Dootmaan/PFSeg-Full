...
class GuidedBraTSDataset3D(torch.utils.data.Dataset):
    def __init__(self,path,mode='train',augment=True,cropping_method='selective',downsample_times=2):
        self.data=[]
        self.label_seg=[]
        self.label_sr=[]
        self.mode=mode
    ...
    def cropMR(self,img,mask):
        ...
        return self.normalization(ndimage.interpolation.zoom(img,[img_size[0]/155,img_size[1]/192,img_size[2]/192],order=1)),self.normalization(ndimage.interpolation.zoom(mask,[128/155,1,1],order=0)),self.normalization(ndimage.interpolation.zoom(img,[128/155,1,1],order=1))
    ...
    def crop_guidance(self,label_sr):
        ...
        # start_time=time.time()
            # 1. random crop
            start_z=random.randint(24,128-guide_img_size[0]-24)
            start_x=random.randint(24,192-guide_img_size[1]-24)
            start_y=random.randint(24,192-guide_img_size[2]-24)
        ...
        # 2. selective search. early stop if variance continously decrease in 10 areas
            ...
            for r in np.linspace(np.sqrt(D**2+M**2+N**2)/r_iter,np.sqrt(D**2+M**2+N**2),r_iter):
                for theta in np.linspace(np.pi/2,np.pi,pi_step):
                    for phi in np.linspace(0,np.pi*2-np.pi*2/8,8):
    ...
    def augment(self,img,label_seg,label_sr):
        ...
        if random.random()<0.5:  #Shift
            ...
            M_label=np.float32([[0,1,self.downsample_times*horizon],[1,0,self.downsample_times*vertical]])
            for i in range(img.shape[0]):
        ...
        # return img[start_z:(start_z+crop_size[0]),start_x:(start_x+crop_size[1]),start_y:(start_y+crop_size[2])],label_seg[self.downsample_times*start_z:self.downsample_times*(start_z+crop_size[0]),self.downsample_times*start_x:self.downsample_times*(start_x+crop_size[1]),self.downsample_times*start_y:self.downsample_times*(start_y+crop_size[2])],label_sr[self.downsample_times*start_z:self.downsample_times*(start_z+crop_size[0]),self.downsample_times*start_x:self.downsample_times*(start_x+crop_size[1]),self.downsample_times*start_y:self.downsample_times*(start_y+crop_size[2])]
